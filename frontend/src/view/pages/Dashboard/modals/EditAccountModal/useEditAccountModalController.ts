...
export default function useEditAccountModalController() {
    ...
    const { isPending, mutateAsync: updateAccount } = useMutation(
        { mutationFn: async (data: UpdateBankAccountParams) => { return bankAccountService.update(data) } }
    )
    const { isPending: isLoadingDelete, mutateAsync: removeAccount } = useMutation(
        { mutationFn: async (id: string) => { return bankAccountService.remove(id) } }
    )
    ...
    return {
        ...
        isDeleteModalOpen, handleOpenDeleteModal, handleCloseDeleteModal, handleDeleteAccount, isLoadingDelete
    };
}
