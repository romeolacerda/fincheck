...
export function useNewTransactionModalController() {
    ...
    const { isPending: isLoading, mutateAsync } = useMutation(
        { mutationFn: async (data: CreateTransactionsParams) => { return transactionsService.create(data) } }
    )
    ...
    const handleSubmit = hookFormHandleSubmit(async (data) => {
        ...
        try {
            ...
            toast.error(newTransactionType === 'EXPENSE' ? "Erro ao cadastradar despesa!" : "Erro ao cadastradar receita!");
            reset();
        }
    });
